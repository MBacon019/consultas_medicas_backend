...
class RegisterTests(TestCase):
    ...
    def test_register_devuelve_jwt(self):
    ...
    def test_register_contrasenas_no_coinciden(self):
        self.data['password2'] = 'Diferente!'
        resp = self.client.post(self.url, self.data)
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_register_username_duplicado(self):
        create_user('juanito')
        resp = self.client.post(self.url, self.data)
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_register_email_duplicado(self):
        create_user('otro', email='juanito@test.com')
        resp = self.client.post(self.url, self.data)
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_register_contrasena_corta(self):
